...
//Colliding function using math and vertex's..
bool Rigidbody::IsColliding(const Rigidbody & rbA, const Rigidbody & rbB)
{
	...
	for (unsigned int i = 0; i < axes.size(); i++) {
		...
		//Get the min and max from your projected scalars
		//b vector of floats
		...
		if (!(bMin <= aMax && bMax >= aMin)) {
			//If we find a gap of light flowing through..
			return false;
		}
	}
	return true;
